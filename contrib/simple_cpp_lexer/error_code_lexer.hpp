...
=============================================================================*/
#if !defined(ERROR_CODE_LEXER_HPP)
#define ERROR_CODE_LEXER_HPP
...
namespace boost { namespace spirit { namespace repository {
    ...
    struct error_code_lexer_2
        : grammar<error_code_lexer_2<Actions> >
    {
        error_code_lexer_2(Actions& actions)
            : actions(actions) {}

        template <typename ScannerT>
        struct definition
        {
            definition(error_code_lexer_2 const& self)
            {
                Actions& actions = self.actions;
    ...
    struct error_code_lexer
        : grammar<error_code_lexer<Actions> >
    {
        error_code_lexer(Actions& actions)
            : actions(actions) {}

        template <typename ScannerT>
        struct definition
        {
            definition(error_code_lexer const& self)
            {
                Actions& actions = self.actions;
                ...
                comment
                    =   as_lower_d["messagetext:"] >> *(space_p | "//") >> *(alnum_p | blank_p | graph_p)
                    ;
            ...
            rule<ScannerT>
                program, preprocessor, comment, identifier;
